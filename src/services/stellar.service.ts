...
export class StellarService {
  ...
  async submitTransaction(
    ...
    try {
      ...
      return result; // Return the full response instead of just the hash
    } catch (error) {
      if (error instanceof Error) {
        ...
        throw new Error("Unknown error during transaction submission");
      }
    }
  }
  ...
  async payment(
    ...
    const asset = this.getAsset(
      assetCode,
      "GAOH2T66FYVBWBR6AZEZITGPQXCYKQCUJCHICBDI66WVWWTWZ4CRKVED",
    );
